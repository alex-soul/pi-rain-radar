import {connectionSaved} from './onboarding.js';
import {refineReview} from './refinements.js';
import {shared,change} from './shared.js';
import {cases,fields,frameFor} from './model.mjs';
...
const banner=make('<aside id="review-banner"><strong>RC3 UI review · synthetic data</strong><button type="button" id="review-cases-toggle" aria-expanded="false">Review cases</button><a href="http://127.0.0.1:3091/__dev" target="scenario-studio">Original studio ↗</a></aside>');document.body.append(banner);
const casePanel=make(`<aside id="review-cases" hidden><label for="review-scenario">Synthetic condition</label><select id="review-scenario">${Object.entries(cases).map(([id,label])=>`<option value="${id}">${label}</option>`).join('')}</select><p>No provider calls. Use <code>demo-token</code> for HA and 32 zeroes for OWM.</p><p id="review-case-help">Configure HA and choose its mappings to test mixed weather.</p></aside>`);document.body.append(casePanel);
$('review-cases-toggle').onclick=()=>{casePanel.hidden=!casePanel.hidden;$('review-cases-toggle').setAttribute('aria-expanded',String(!casePanel.hidden));};
$('review-scenario').onchange=async()=>{await change({action:'scenario',scenario:$('review-scenario').value});};
...
$('review-ha-save').onclick=async()=>{try{await change({action:'ha',url:$('review-ha-url').value,token:$('review-ha-token').value});$('review-ha-token').value='';$('review-ha-note').textContent='Connected · synthetic HA';await connectionSaved('ha');}catch(e){$('review-ha-note').textContent=e.message;}};
$('review-ha-remove').onclick=async()=>{await change({action:'remove-ha'});};
$('review-ha-collect').onchange=()=>change({action:'settings',haCollect:$('review-ha-collect').checked});
...
$('review-owm-collect').onchange=()=>change({action:'settings',owmCollect:$('review-owm-collect').checked});
...
function sync(){
 $('review-scenario').value=shared.scenario;
 $('review-case-help').textContent=shared.scenario==='history'?'Choose Archive, a 24-hour window and Show. The simulated source/unit boundary is '+new Date(shared.historyBoundary).toLocaleString()+'. Before it: °C/mph and temperature fallback; after it: °F/km/h and unit-mismatch fallback. Compare OpenWeather.':'Configure HA with demo-token, enable collection, select Home Assistant and map readings to test this condition.';
 ...
 const model=frameFor(shared,Date.now(),false);
 for(const f of fields){$('review-map-'+f).value=shared.mappings[f];$('review-map-'+f).querySelector('[value="ha"]').disabled=!shared.ha;const row=model.rows[f];$('review-unit-'+f).textContent=row.reason.startsWith('Unit mismatch')?row.reason:shared.mappings[f]==='owm'&&!shared.owm?'OpenWeather is not configured. This reading will be a gap.':'';}
 const ha=!shared.ha?'Not configured':shared.scenario==='auth'?'Authentication failed':shared.scenario==='offline'?'Unreachable':'Connected';
 health('review-ha-health',ha,ha==='Connected'?'ready':!shared.ha?'neutral':'error');$('review-ha-note').textContent=ha;
 const camera=!shared.camera?'Not configured':!shared.cameraEnabled?'Collection disabled':shared.camera.mode==='ha'&&(!shared.ha||['offline','auth'].includes(shared.scenario))?'Camera unavailable':'Collecting';
 health('review-camera-health',camera,camera==='Collecting'?'ready':camera==='Camera unavailable'?'error':'neutral');
 ...
 if(shared.camera){$('review-camera-name').textContent=shared.camera.name;$('review-camera-kind').textContent=shared.camera.mode==='ha'?'Home Assistant':'Direct snapshot';document.querySelector('#review-camera-row img').src='/__review/camera.jpg?portrait='+Number(shared.camera.portrait);}
}
refineReview();
window.addEventListener('review-change',sync);sync();
