...
export async function createDevCamera(directory,scenario,origin,seedEnd){
  ...
  const get=async(value,{signal})=>{
    ...
    if(!preparing&&scenario()==='camera-auth')throw new CameraError('CAMERA_AUTH');
    if(!preparing&&scenario()==='camera-slow')await new Promise((_,reject)=>{
    ...
    if(!preparing&&scenario()==='camera-invalid')return Buffer.from('synthetic invalid image');
    ...
    if(!preparing&&scenario()==='camera-stale'){
