...
export function setupCameraSettings(canEdit,request){
 ...
 let tested=false,epoch=0,ticket=null,blob=null,busy=false,thumbnailBlob=null;
 ...
 function invalidate(){tested=false;ticket=null;$('review-camera-commit').disabled=true;}
 ...
 $('review-camera-commit').onclick=async()=>{if(!tested||!canEdit()||busy)return;busy=true;
  try{await settingsRequest('/camera',{ticket,enabled:shared.cameraEnabled});await refresh();await loadThumbnail();editor.close();$('review-camera-note').textContent=shared.cameraEnabled?'Camera saved. Collecting.':'Camera saved. Collection disabled.';}catch(e){$('review-camera-feedback').textContent=e.message;}finally{busy=false;}
 };
