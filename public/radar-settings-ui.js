...
export function setupRadarSettings(canEdit,request) {
  ...
  panel.innerHTML='<div class="api-key-heading"><span>RainViewer API key</span><a class="api-setup-link" href="https://github.com/alex-soul/pi-rain-radar/blob/main/THIRD_PARTY_NOTICES.md" target="_blank" rel="noopener noreferrer">(Usage &amp; limits)</a></div><p>RainViewer generously provides public API access without a key. Data is cached and shared across displays to respect rate limits.</p>';
  $('rainbow-config').before(panel);
  $('rainbow-key').after($('rainbow-key-note'));
  const actions=$('radar-apply').closest('.weather-key-actions');actions.before($('radar-settling').closest('label'),$('radar-settling-note'));
  ...
  async function key(remove){
    if(!canEdit()||busy)return;const epoch=generation,apiKey=remove?'':$('rainbow-key').value.trim();
  ...
  $('radar-apply').onclick=async()=>{
    ...
    try{const response=await request('/radar',{waitForSettle:$('radar-settling').checked,main:$('radar-main-source').value,overview:$('radar-overview-source').value,monthlyLimit:$('rainbow-cap').checked?Number($('rainbow-limit').value):null});const result=await response.json();if(epoch!==generation)return;$('radar-source-note').textContent=response.ok?'Sources applied.':result.error||'Could not apply sources.';}
  ...
  return {
    clear(){generation++;$('rainbow-key').value='';},
    async load(radar){
      const epoch=++generation;$('radar-main-source').value=radar?.main??'rainviewer';$('radar-overview-source').value=radar?.overview??'same';$('rainbow-cap').checked=radar?.monthlyLimit!=null;$('rainbow-limit').value=radar?.monthlyLimit??28000;$('radar-source-note').textContent='';estimate();
