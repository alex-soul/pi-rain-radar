...
const severity={disabled:0,unknown:0,ready:1,warning:2,error:3};
...
export function availabilityRows({frames=[],data={},status={},archive=false,start,end}){
 ...
 function add(label,enabled,fn){if(!enabled)return;rows.push({label,segments:times.map((time,i)=>{const period=data.collectionPeriods?.findLast(p=>p.time<=time),disabled=!['Camera','Clouds'].includes(label)&&period?.[label.toLowerCase()]===false;const detail=disabled?[{label,health:'disabled'}]:fn(time,i);return {time,health:detail.every(d=>['unknown','disabled'].includes(d.health))&&detail.some(d=>d.health==='unknown')?'unknown':worst(detail.map(d=>d.health)),detail};})});}
