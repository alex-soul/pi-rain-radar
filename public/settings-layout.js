...
function sync(){
 ...
 const camera=!shared.camera?'Not configured':!shared.cameraEnabled?'Collection disabled':shared.camera.error??(shared.camera.fresh?'Collecting':shared.camera.state==='stale'?'Snapshot is stale':'Waiting for snapshot');
 health('review-camera-health',camera,camera==='Collecting'?'ready':shared.camera?.error?'error':'neutral');
 $('review-camera-interval').value=String(shared.camera?.intervalMinutes??5);$('review-camera-interval').disabled=!shared.camera;
