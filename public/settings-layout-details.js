...
export function setupReviewRadar(){
 ...
 function paint(){
  ...
  for(const [id,configured,enabled]of [['owm',shared.owm,shared.owmCollect],['ha-collection',shared.ha,shared.haCollect]]){
   const node=$('review-'+id+'-status');const failed=id==='ha-collection'&&!!shared.presentation?.error;node.textContent=!configured?'Not configured':!enabled?'Disabled':failed?'Enabled · Error':'Enabled';node.dataset.health=configured&&enabled?(failed?'error':'ready'):'neutral';
  }
 }
