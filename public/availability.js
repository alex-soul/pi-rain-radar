...
export function updateAvailability(input,onSelect){
 ...
 row.segments.forEach((segment,i)=>{const b=document.createElement('button');b.type='button';b.dataset.health=segment.health;b.setAttribute('aria-label',`${row.label} ${clock(segment.time)} · ${segment.health==='ready'?'Available':segment.health==='warning'?'Earlier observation':segment.health==='error'?'Missing':segment.health==='pending'?'Pending':segment.health==='unknown'?'Unknown':'Disabled'}`);const show=()=>{active=b;const out=$('availability-detail');out.textContent=`${row.label} · ${clock(segment.time)} · `+segment.detail.map(d=>`${d.label}: ${d.health==='ready'?'available':d.health==='warning'?'reused '+clock(d.time):d.health==='error'?'missing':d.health==='pending'?'pending':d.health==='unknown'?'unknown':'disabled'}`).join(' · ');out.hidden=false;const rect=b.getBoundingClientRect();out.style.left=Math.max(8,Math.min(rect.left,window.innerWidth-out.offsetWidth-8))+'px';out.style.top=Math.max(8,rect.top-out.offsetHeight-10)+'px';};
 ...
 root.style.setProperty('--availability-count',String(steps+1));
