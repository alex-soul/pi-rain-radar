...
export async function createRainbow(directory, {
  initialKey = null, usageDirectory = null, cloudReserve = () => 0, request = fetch, now = Date.now, sleep = ms => new Promise(resolve => setTimeout(resolve, ms)),
  ...
  const keyFile = join(folder, 'rainbow.json'), usageFile = join(usageDirectory??folder, 'rainbow-usage.json');
  if(usageDirectory)await mkdir(usageDirectory,{recursive:true,mode:0o700});
  let key = '', ledger = { month: monthOf(now()), tiles: 0, requests: 0, total: 0, retryAt: 0 };
  try {
    const saved = initialKey===null?JSON.parse(await readFile(keyFile, 'utf8')):{apiKey:initialKey};
  ...
  async function call(path, apiKey, tile = false, cloud = false) {
    return serial(async () => {
      ...
      if(cloud&&limit!==null&&requests+cloudReserve()>=limit)throw new RainbowError('limit','Clouds paused to reserve remaining allowance for rain.');
      ...
      if (!response.ok) {
        ...
        if(response.status===404)throw new RainbowError('not-found','Rainbow frame unavailable.');
  ...
  async function snapshot(apiKey,layer='precip') {
    let value;
    try { value = JSON.parse((await call('/tiles/v1/snapshot?layer='+layer, apiKey,false,layer==='clouds')).toString()).snapshot; }
  ...
  async function tileAt(time, tile, apiKey,cloud=false) {
    const { zoom, x, y } = tile;
    ...
    const data = await call(cloud?`/tiles/v1/clouds/${time}/${zoom}/${x}/${y}`:`/tiles/v1/precip/${time}/0/${zoom}/${x}/${y}?color=8&coverage=0`, apiKey, true,cloud);
    try {
      if (!cloud && data.subarray(0, 8).toString('hex') !== '89504e470d0a1a0a') throw new Error();
      const info = await sharp(data, { limitInputPixels: 65536 }).metadata();
      if (info.width !== 256 || info.height !== 256 || !['png','webp'].includes(info.format)) throw new Error();
  ...
  return {
    ...
    cloudSnapshot:()=>acquisition(apiKey=>snapshot(apiKey,'clouds')),
    cloudTile:(time,tile)=>acquisition(apiKey=>tileAt(time,tile,apiKey,true)),
    ...
    async configure(value) {
      ...
      if(!value&&!configuring){try{await onNewKey();await queue;}catch{return {status:409,error:'Could not stop collections. Try again shortly.'};}}
