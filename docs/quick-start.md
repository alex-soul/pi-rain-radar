...
2. **API Keys, optional:** paste your [OpenWeather One Call 3.0](https://openweathermap.org/api/one-call-3) key and Save key. This adds current readings and MinuteCast. Check the provider's access/pricing and account limit. After moving the map, allow 10–15 minutes for the next weather request. Radar works without a key.
