...
2. **System → API → Weather, optional:** paste your [OpenWeather One Call 4.0](https://openweathermap.org/api/one-call-4) key and Save key. This adds current readings and Rain forecast. Activate the separate 4.0 subscription first, even if you already use 3.0. Check the provider's access/pricing and set the daily limit to 1,000 to stay within its currently advertised free allowance; the default 2,000 limit permits charges. After moving the map, allow 10–15 minutes for the next weather request. Radar works without a key.
