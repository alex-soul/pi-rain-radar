...
const source=(await readFile(new URL('../public/diagnostics.js',import.meta.url),'utf8')).replace('export function','function');
...
test('diagnostics requests stop when hidden or closed and discard late results',async()=>{
  ...
  const context={dialog,request:()=>{calls++;return new Promise(resolve=>resolveRequest=resolve);},canEdit:()=>editable,
  ...
  resolveRequest({ok:true,json:async()=>({events:[],limit:25})});
  await new Promise(resolve=>setImmediate(resolve));
  assert.equal(note.textContent,'Last 25 events since restart');assert.equal(timers.size,1);
