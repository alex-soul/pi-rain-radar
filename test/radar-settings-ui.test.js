...
function fixture() {
  ...
  function element(){return {value:'',checked:false,disabled:false,open:false,handlers:{},className:'',set id(v){nodes.set(v,this);},set innerHTML(v){},before(){},after(){},append(){},closest(){return element();},addEventListener(k,v){this.handlers[k]=v;},replaceChildren(){},reportValidity(){return true;},showModal(){this.open=true;},close(){this.open=false;},click(){return this.onclick?.();}};}
  function get(id){if(id==='fixture-toggle')return null;if(!nodes.has(id))nodes.set(id,element());return nodes.get(id);}
  const context={document:{getElementById:get,createElement:element,createTextNode:v=>v,body:{append(){}}}};
