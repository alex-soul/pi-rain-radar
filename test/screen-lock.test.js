...
function fixture(saved='false') {
  const handlers={}, documentHandlers={}, nodes={}, classes=new Set();let editable=true, stored=saved;
  function node(id) { return nodes[id]??={handlers:{},attributes:{},dataset:{},checked:false,hidden:false,
    ...
    matches(){return this.required;},focus(){},showModal(){this.open=true;},close(){this.open=false;this.handlers.close?.();}}; }
  ...
  const ctx=vm.createContext({document:{body:{classList:{contains:k=>classes.has(k),toggle(k,v){v?classes.add(k):classes.delete(k);}}},getElementById:node,querySelector:node,querySelectorAll:()=>[optional,required],addEventListener:(k,fn)=>documentHandlers[k]=fn},window:{addEventListener:(k,fn)=>handlers[k]=fn,dispatchEvent(){}},localStorage:{getItem:()=>stored,setItem:(k,v)=>stored=v},Event,URL,setTimeout,canEdit:()=>editable});
  vm.runInContext(source+'\nsetupScreenLock(canEdit);',ctx);
  return {handlers,nodes,node,classes,optional,required,stored:()=>stored,editable:v=>editable=v};
}
