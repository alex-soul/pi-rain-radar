...
test('gap explanations never seek; playback controls preserve popup while outside dismisses',async()=>{
 ...
 node('playback-row').append(node('play'),node('timeline'),node('frame-total'));
 ...
 node('frame-total').onclick();assert.equal(node('availability-panel').hidden,false);
 for(const target of [node('play'),node('timeline')]){events.pointerdown({target});assert.equal(node('availability-panel').hidden,false);}
